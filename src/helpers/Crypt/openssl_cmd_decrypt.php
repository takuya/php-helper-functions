...
if ( !function_exists( __NAMESPACE__.'\\openssl_cmd_decrypt' ) ) {
  function openssl_cmd_decrypt ( string $data, string $pass, $iter = 1000*1000, $cipher = '-aes-256-cbc' ): string {
    // build command
    $cmd = csplit( 'openssl enc -d %CIPHER% -pbkdf2 -iter %COUNT% -in - -out - -k %PASS% -base64', ' ' );
    ...
    fwrite( $stdio[0], $secret_value = file_exists( $data ) ? file_get_contents( $data ) : $data );
    fclose( $stdio[0] );
